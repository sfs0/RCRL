...
def train1(
    ...
    with tqdm(
        total=len(train_loader), desc=f"Eps {epoch}/{num_epoches}", unit="img"
    ) as pbar:
        for i, (A, B, mask, with_label) in enumerate(train_loader):  # with_label是
            ...
            if use_ema is False:
                ...
                if with_label.any():
                    ...
                    loss = criterion(preds[0], Y[with_label]) + criterion(
                        preds[1], Y[with_label]
                    )
                    Y = Y[with_label]
                else:
                    """
                ...
                preds = net(A, B)
                if with_label.any():
                    loss = criterion(preds[0][with_label], Y[with_label]) + criterion(
                        preds[1][with_label], Y[with_label]
                    )
                else:
                    loss = 0
            ...
            if use_ema is True:
                ...
                with torch.no_grad():
                    z1 = A[~with_label]
                    z2 = B[~with_label]
                    pseudo_attn, pseudo_preds = ema_net(
                        z1, z2
                    )  # 分别是两个输出attention_map是中间层知识,prediction是预测结果

                    # pseudo_attn,pseudo_preds =  ema_net(A[~with_label], B[~with_label]) #分别是两个输出attention_map是中间层知识,prediction是预测结果
                    pseudo_attn, pseudo_preds = (
                        torch.sigmoid(pseudo_attn).detach(),
                        torch.sigmoid(pseudo_preds).detach(),
                    )

                ### 修改后  置信度门控-----------
                # 自适应阈值计算
                threshold = 0.7 + (0.95 - 0.7) * (epoch / num_epoches)

                # 创建置信度掩码
                conf_mask_attn = (pseudo_attn > threshold) | (
                    pseudo_attn < 1 - threshold
                )
                conf_mask_preds = (pseudo_preds > threshold) | (
                    pseudo_preds < 1 - threshold
                )
                valid_pixels = conf_mask_attn & conf_mask_preds

                # 确保梯度流不会中断
                if valid_pixels.any():
                    # 保持原始张量结构，只计算有效像素的损失
                    valid_mask = torch.zeros_like(
                        preds[0][~with_label], dtype=torch.bool
                    )
                    valid_mask[valid_pixels] = True

                    # 使用掩码计算损失
                    loss_semi = semicriterion(
                        preds[0][~with_label][valid_mask], pseudo_attn[valid_mask]
                    ) + semicriterion(
                        preds[1][~with_label][valid_mask], pseudo_preds[valid_mask]
                    )
                else:
                    # 创建需要梯度的0值张量
                    loss_semi = torch.zeros(1, device=A.device, requires_grad=True)[0]
                # ====== 结束修复 ======

                # # ------------
                # loss_semi = semicriterion(
                #     preds[0][~with_label], pseudo_attn
                # ) + semicriterion(
                #     preds[1][~with_label], pseudo_preds
                # )  # 测试这里的效果，如果有用则方便讲故事
                ...
                # loss = (
                #     loss + 0.2 * loss_semi
                # )  # 全监督损失+半监督损失，半监督系数默认为0.2，测试0.3，0.4，0.5！！
                # # 动态调整半监督损失权重 - 随着训练进行逐渐增加
                if use_ema:
                    # 根据当前epoch动态计算半监督权重
                    semi_weight = 0.2 + 0.2 * min(1.0, (epoch - 10) / 40.0)  # 从epoch 10开始，逐渐增加到0.4
                    loss = loss + semi_weight * loss_semi
                else:
                    loss = loss  # 仅使用有标签数据时，不加半监督损失

                Eva_train2.add_batch(
                    mask[~with_label].cpu().numpy().astype(int),
                    (preds[1][~with_label] > 0).cpu().numpy().astype(int),
                )  # ~相反
            # ---- loss function ----
            ...
            ### 修改后  动态ema--------
            def get_alpha(epoch, total_epochs, min_alpha=0.9, max_alpha=0.999):
                """动态调整EMA衰减系数"""
                # 线性增长策略
                return min_alpha + (max_alpha - min_alpha) * (epoch / total_epochs)

            with torch.no_grad():
                update_ema_variables(net, ema_net, alpha=0.99)  # 0.9，0.995，0.999
                # -------------修改 动态ema
                # current_alpha = get_alpha(epoch, num_epoches)
                # update_ema_variables(net, ema_net, alpha=current_alpha)
                # -------------
            # --------------
    ...
    print(
        ...
        % (
            ...
            num_epoches,
            train_loss,
            Eva_train2.Intersection_over_Union()[1],
            Eva_train2.Precision()[1],
            Eva_train2.Recall()[1],
            Eva_train2.F1()[1],
        )
    )
    ...
    if use_ema is True:
        print(
            "Epoch [%d/%d],\n[Training]IoU: %.4f, Precision:%.4f, Recall: %.4f, F1: %.4f"
            % (epoch, num_epoches, IoU, Pre, Recall, F1)
        )
    ...
    csv_data = [
        epoch,
        train_loss,  # 训练损失
        Eva_val.Intersection_over_Union()[1],  # 验证集IoU
        Eva_val.Precision()[1],  # 验证集Precision
        Eva_val.Recall()[1],  # 验证集Recall
        Eva_val.F1()[1],  # 验证集F1
        Eva_val2.Intersection_over_Union()[1],  # 教师模型验证IoU
        Eva_val2.Precision()[1],  # 教师模型验证Precision
        Eva_val2.Recall()[1],  # 教师模型验证Recall
        Eva_val2.F1()[1],  # 教师模型验证F1
    ]
    ...
    print(
        "[Ema Validation] IoU: %.4f, Precision:%.4f, Recall: %.4f, F1: %.4f"
        % (
            Eva_val2.Intersection_over_Union()[1],
            Eva_val2.Precision()[1],
            Eva_val2.Recall()[1],
            Eva_val2.F1()[1],
        )
    )
    ...
    if new_iou > best_iou:
        ...
        torch.save(
            ema_net.state_dict(), save_path + "_train1_" + "_best_teacher_iou.pth"
        )  # 当student的精度最高的时候，同时存teacher的精度，然后用teacher的精度进行测试
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batchsize", type=int, default=16, help="training batch size"
    )  # 修改这里！！！
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.05,
        # default=0.1,
        help="Proportion of the labeled images",
    )  # 修改这里！！！
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpu_id", type=str, default="0,1", help="train use gpu"
    )  # 修改这里！！！
    parser.add_argument(
        "--data_name",
        type=str,
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="SemiModel_noema04",
        help="the test rgb images root",
    )
    parser.add_argument(
        "--save_path", type=str, default="./output/RCRL-SemiCD/LEVIR-5-1-99/"
    )  # 半监督的模型保存路径！！
    ...
    if opt.data_name == "LEVIR":
        opt.train_root = "/root/autodl-tmp/LEVIR_CD_256/train/"
        opt.val_root = "/root/autodl-tmp/LEVIR_CD_256/val/"
    elif opt.data_name == "WHU":
        opt.train_root = "/root/autodl-tmp/WHU-CD256-HANet/train/"
        opt.val_root = "/root/autodl-tmp/WHU-CD256-HANet/val/"
    elif opt.data_name == "GoogleGZ":
        opt.train_root = "/root/autodl-tmp/GZ-CD256/train/"
        opt.val_root = "/root/autodl-tmp/GZ-CD256/val/"

    train_loader = data_loader.get_semiloader(
        opt.train_root,
        opt.batchsize,
        opt.trainsize,
        opt.train_ratio,
    ...
    # 创建CSV文件并写入表头
    csv_file_path = os.path.join(save_path, f"training_metrics_{opt.model_name}.csv")
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        if epoch <= 0:  # 默认的为5，测试10，15，20
            use_ema = False
            # print('labeled ration=0.05,Ablation现在监督训练的次数为:20!')
            # print('labeled ration=0.1,Ablation现在监督训练的次数为:20!')
        else:
            use_ema = True
